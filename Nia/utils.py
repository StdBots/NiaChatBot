...
async def log_to_channel(bot: Bot, event_type: str, details: dict):
    ...
    headers = {
        "start": "🌸 SYSTEM ONLINE",
        "join": "🥂 NEW GROUP JOINED",
        "leave": "💔 LEFT GROUP",
        "command": "👮 ADMIN COMMAND",
        "transfer": "💸 TRANSACTION"
    }

    header = headers.get(event_type, "📜 LOG ENTRY")
    ...
    if 'link' in details:
        ...
        if link_val and link_val.startswith("http"):
            text += f"🔗 <b>Invite:</b> <a href='{link_val}'>Click to Join</a>\n"
        else:
            text += f"🔒 <b>Invite:</b> Hidden/Private\n"
...
async def resolve_target(update, context, specific_arg=None):
    ...
    if query.isdigit():
        ...
        if doc:
            return doc, None
        return None, f"❌ <b>{BOT_NAME}</b> ID <code>{query}</code> not found."
    ...
    return None, f"❌ <b>Oops!</b> User <code>@{clean_username}</code> has not started me."
...
def get_active_protection(user_data):
    try:
        ...
        if partner_id:
            ...
            if partner:
                partner_expiry = partner.get("protection_expiry")
